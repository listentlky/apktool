...
public class Main {
    ...
    private static void cmdDecode(CommandLine cli) throws AndrolibException {
        ...
        List<DecodeApkFileModel> apkFileList = decoder.getApkFileList();
        apkFileList.add(new DecodeApkFileModel.Builder()
            .setApkFileName(new File(apkName))
            .setApkOutDir(outDir)
            .build());

        for (String arg:cli.getArgs()){
            if(arg.equalsIgnoreCase("config.txt")){
                QuickConfig.getInstance().initQuickConfig(arg);
                QuickConfig.getInstance().setApkFileList(apkFileList);
            }
        }

        for (int i = 0;i<apkFileList.size();i++) {
            if(i == apkFileList.size()-1){
                QuickConfig.getInstance().setAutoBuildQuick(true);
            }else {
                QuickConfig.getInstance().setAutoBuildQuick(false);
            }
            DecodeApkFileModel decodeApkFileModel = apkFileList.get(i);
            if(!TextUtils.isEmpty(decodeApkFileModel.getQuickMark())){
                System.out.println("decode 渠道: "+decodeApkFileModel.getQuickMark());
            }
            decoder.setOutDir(decodeApkFileModel.getApkOutDir());
            decoder.setApkFile(decodeApkFileModel.getApkFileName());
            try {
                decoder.decode();
            ...
            //    System.exit(1);
                ...
                System.err.println("Input file (" + decodeApkFileModel.getApkFileName() + ") " + "was not found or was not readable.");
            //    System.exit(1);
                        ...
                        for (String quick : QuickConfig.getInstance().getQuickModel().getQuick().getOut()) {
                            ...
                            for (QuickInfoModel quickInfoModel : QuickConfig.getInstance().getQuickModel().getQuick().getInfo()) {
                                if (quick.equals(quickInfoModel.getSdk())) {
                                    System.out.println("读取当前渠道配置 :"+quickInfoModel.toString());
                                    QuickConfig.getInstance().setCurrentQuickInfo(quickInfoModel);
                                }
                            }
